...
- 나쁜 기준: "그래프가 잘 나온다"
- 좋은 기준: "7일 엣지 5개 / 30일과 전체 8개 / `value=0` 엣지 0건 / 떠다니는 엣지 0건"
- 버그 수정이면 **재현 조건을 먼저 확정**하고, 고친 뒤 그 조건에서 사라졌는지 확인하세요

🔴 **예외는 `try`와 `except`로 제대로 받는다 (2026-09-14 사용자 지시).** Python은 `catch`가 아니라 `except`입니다.

- **좁은 것부터 넓은 것 순으로 쌓으세요.** `except Exception` 하나로 다 잡으면 **재시도해도 되는 것(429, 5xx, 네트워크)과 안 되는 것(400, 404)이 구분되지 않습니다.**
- ⚠️ **삼킨 예외가 가장 위험합니다.** `except: pass`는 절대 쓰지 마세요. 이 저장소가 반복해서 겪은 실패가 전부 **에러 없이 조용히 잘못 도는** 유형입니다(Django 템플릿 주석, Docker Compose의 `$` 절단, 크롤러가 메뉴 문자열을 본문으로 판정). **예외를 삼키면 그 목록에 하나를 더 보태는 것입니다.**
- **`else`와 `finally`를 쓰세요.** `else`는 예외가 안 났을 때만 돌아서 `try` 블록을 좁게 유지해 주고, `finally`는 정리를 보장합니다.
- **외부 호출에는 반드시 붙입니다** — 네이버 API, Bedrock, 크롤링, DB 연결. ⚠️ **다만 위 「최소 구현」과 충돌하지 않게 하세요.** 일어날 수 없는 경우를 위한 방어는 넣지 않습니다. **실제로 나는 예외만** 받습니다.
- **건별로 도는 루프에서는 한 건의 실패가 전체를 멈추지 않게** 하되, **실패 건수를 세어 보고**하세요. 조용히 넘어가면 몇 건이 빠졌는지 아무도 모릅니다.

```python
try:
    r = client.messages.create(...)
except anthropic.RateLimitError:        # 재시도 가능
    ...
except anthropic.APIStatusError as e:   # 4xx는 재시도해도 같다
    ...
except anthropic.APIConnectionError:    # 네트워크
    ...
else:
    ...                                  # 예외가 안 났을 때만
finally:
