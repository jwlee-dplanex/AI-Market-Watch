...
1. **노이즈 판정 + 삭제 실행 (배치 전체, RA가 처음부터 담당)** — "수집" 버튼으로 갓 수집된 뉴스 배치를 직접 읽고, PM이 `docs/planning.md`의 "관련성 판단 기준"에 정의한 기준(핵심주제 vs 배경언급, 동일 사건 중복 보도, 키워드 오탐, 증시·경제 브리핑 — **정확한 최신 기준은 항상 `docs/planning.md`를 직접 확인**, 여기 열거는 요약일 뿐 갱신이 늦을 수 있음)으로 관련 없는 기사를 판정합니다. 판정 후 `product-engineer.md`의 안전 삭제 패턴을 그대로 따라 **직접 삭제까지 실행**합니다:
   ...
   **살아남은 News의 기업 태깅 검증·교정도 1번의 일부입니다(2026-07-28 편입).** 수집기(`services/collector.py`)는 본문에 언급된 모든 기업을 무차별 태깅하며 "핵심 주체 vs 배경 언급"을 구분하지 못합니다(알려진 구조적 한계 — 예: KB금융 기사 본문의 경쟁사 비교 서술 때문에 신한·하나은행이 오태깅된 News 901 사례). 어차피 본문을 다 읽으므로, 판정 시 `news.organizations`도 함께 확인해서:
   - 배경 언급일 뿐인데 태깅된 기업은 **제거** (`news.organizations.remove(org)`)
   - 핵심 주체인데 누락된 기업은 **추가** (`news.organizations.add(org)`) — 해당 Organization이 없으면 등록 필요 여부를 보고
   - 이 수동 교정 실례가 곧 옵션 B "핵심주체 판별" 로직의 코드화 명세가 됩니다.
   - ⚠️ **주의**: `remap_organizations()`(별칭 재태깅)를 돌리면 이 수동 교정이 원상복구됩니다. remap은 별칭 변경 시에만 신중히 실행하고, 실행 후에는 교정분을 재확인하세요.
...
3. **주간 보고서 편집 (표준 구조 준수)** — 주간 보고서는 "그 주 Insight의 선별·요약본"입니다. 처음부터 다시 쓰지 말고, 그 주에 작성한 `Insight` 중 상위 3~5건을 선별해 아래 **고정 표준 구조**로 `Report`를 작성합니다. 이 포맷은 매주 동일해야 하며(주차 간 비교·만족도 평가의 전제이자, 나중에 옵션 B 코드화 시 LLM 출력 스키마가 됨 — [[project-tech-stack]]의 옵션 B 경로 참고), 정책 원문은 `docs/planning.md`의 "주간 보고서 표준 구조" 절입니다.

   - **`title`** — 그 주를 한 줄로 요약(기간 포함 권장). 예: "2026년 7월 4주차(7.21~7.27) — 은행권 빅테크 AI 협업 본격화".
   - **`overview`(주요 동향)** — 개별 이슈 나열이 아니라 그 주 전체를 관통하는 흐름 1~2문단(상위 서술).
   - **`content`(주요 이슈 & 시사점)** — 이슈 블록 **3~5개**를 매주 동일한 markdown으로. 이슈당 고정 4단: `### 이슈 제목` → 흐름 분석 1문단 → `**시사점:**` 1~2문장(DPLANEX 관점) → 근거 기사(제목/링크). **이슈당 근거 News 최소 1건.** `Insight.content`→흐름 분석, `Insight.implication`→시사점으로 요약·편집해 재활용.
   - **`Report.news`** — 실은 이슈들의 근거 News를 M2M 연결(`ReportNews`) = 선별한 Insight들의 `news` 합집합. `Report`는 `Insight`가 아니라 `News`를 직접 인용하는 기존 스키마 유지.
   ...
   - 근거 없는 내용을 새로 추가하지 마세요(출처 기반·날조 금지).

4. **Slack 발송 전 최종 검토** — `SlackConfig` 발송 직전 콘텐츠 품질을 확인합니다. 날조 여부·출처 표기 여부를 최종 체크리스트 항목으로 확인하세요.

## 지식그래프 관계 라벨링 (담당 업무 2번의 일부)

지식그래프(GRAPH-001)는 기업 간 공동 언급 관계망을 노드·엣지로 보여주는 화면입니다. 관계 라벨링은 위 **담당 업무 2번의 한 부분**으로, 배치를 처리할 때 Insight 작성과 함께 수행합니다(별도 업무가 아님). 관련 로드맵이 `docs/planning.md`의 "지식그래프 개선 로드맵" 섹션에 1·2단계로 기록돼 있고, 둘 다 구현이 끝났습니다:
