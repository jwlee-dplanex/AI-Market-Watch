...
3. **주간 보고서 편집 (표준 구조 준수)** — 주간 보고서는 "그 주 Insight의 선별·요약본"입니다. 처음부터 다시 쓰지 말고, 그 주에 작성한 `Insight` 중 상위 3~5건을 선별해 아래 **고정 표준 구조**로 `Report`를 작성합니다. 이 포맷은 매주 동일해야 하며(주차 간 비교·만족도 평가의 전제이자, 나중에 옵션 B 코드화 시 LLM 출력 스키마가 됨 — [[project-tech-stack]]의 옵션 B 경로 참고), 정책 원문은 `docs/planning.md`의 "주간 보고서 표준 구조" 절입니다.
   ...
   - **`content`(주요 이슈)** — 이슈 블록 **3~5개**를 매주 동일한 markdown으로. 이슈당 고정 **3단**: `### 이슈 제목` → 흐름 분석 1문단 → 시사점 1~2문장(DPLANEX 관점). **시사점에 `**시사점:**` 같은 라벨을 붙이지 않는다**(2026-07-29 개정 — 라벨 없는 일반 문단으로, "블록의 마지막 문단=시사점"이라는 위치 규칙으로 식별). **content 안에 근거 기사 링크도 넣지 않는다**(근거 표기는 아래 `Report.news` 연결 + 화면 하단 "참고 뉴스" 섹션이 담당). `Insight.content`→흐름 분석, `Insight.implication`→시사점으로 요약·편집해 재활용.
   - **`Report.news`** — 실은 이슈들의 근거 News를 M2M 연결(`ReportNews`) = 선별한 Insight들의 `news` 합집합. **이슈당 근거 News 최소 1건**이라는 요건은 이 M2M 연결로 충족되며, 이 연결이 곧 출처 표기 역할(화면 하단 "참고 뉴스" 목록으로 노출). `Report`는 `Insight`가 아니라 `News`를 직접 인용하는 기존 스키마 유지.
