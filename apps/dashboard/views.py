...
def _build_rolling_buckets(today, earliest_date, bucket_size):
    ...
    ⚠️ 알려진 문제 (2026-08-20 관측, 사용자 판단으로 수정 보류)
    아래 루프는 `end < earliest_date`인 버킷까지 포함하고 멈추므로, 맨 앞 버킷이 데이터
    시작일보다 앞에서 끝날 수 있다. 실측 예: earliest_date=07-04인데 첫 버킷이 06/26~07/02라
    x축 맨 왼쪽 라벨(07/02)이 뉴스가 0건인 구간을 가리킨다 — 차트가 실제보다 일찍 시작한
    것처럼 보이고 왼쪽 끝이 항상 0에서 출발한다.
    지금 두는 이유는 수집이 쌓일수록 이 빈 구간 하나가 전체에서 차지하는 비중이 줄기 때문이다
    (버킷 8개일 땐 1/8이지만 석 달 뒤엔 1/30). 고칠 때는 design.md의 알고리즘도 함께 고쳐야
    하고, 첫 버킷을 잘라 쓰면 그 점만 기간이 짧아 건수가 적게 찍히는 걸 감수해야 한다."""
    buckets = []
    i = 0
    while True:
