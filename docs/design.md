...
|---|---|---|---|
| ① 일별 뉴스 건수 추이 | 동적 `<h3>`(일별/주별/월별 뉴스 건수 추이) 텍스트 옆 | `dashboard.trend` | "선택한 기간의 뉴스 건수를 발행일별로 보여드려요." |
...
2. **최대 높이 + 카드 내 스크롤 (C)** — 두 카드가 **동일한 `max-h-[40rem]`(=640px)** 값을 공유한다. 헤더(제목+툴팁, 최신 뉴스는 "전체 보기" 링크까지)는 `shrink-0`로 카드 상단에 고정하고, 리스트 영역만 `flex-1 min-h-0 overflow-y-auto`로 감싸 남은 세로 공간 안에서 스크롤한다.
   ...
   - **왜 뷰포트 비례(`vh`)가 아니라 고정 px(rem)인가**: 이 로우 위에 "핵심 지표" 섹션(차트+랭킹 2장, 기간 셀렉터 포함)이 있어 뷰포트에서 차지하는 상단 여백이 콘텐츠에 따라 미세하게 달라진다. `calc(100vh - Npx)` 같은 계산식은 그 여백이 어긋나면 매 화면마다 다시 튜닝해야 해 깨지기 쉽고, PE 구현·QA 난도도 올라간다. 고정값은 화면 크기와 무관하게 항상 동일한 스크롤 트리거 지점을 보장해 예측 가능하고 검증하기 쉽다 — 작은 노트북 화면에서는 카드 아래로 페이지 스크롤이 더 생길 수 있지만, 이는 "카드 안 빈 공간"보다 훨씬 자연스러운 트레이드오프라고 판단했다.
...
2. 2단 그리드 (핵심 지표 아래), 두 카드 모두 최대 높이(640px)를 넘으면 헤더는 고정한 채
