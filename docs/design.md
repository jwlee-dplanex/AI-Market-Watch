...
│  ─────────────────────────────────────────────────────────────────────  │
│  고아 라벨 관계 (지식그래프)                                             │
│  0건 ✓조용히 잘 보이고 있어요   (>0건이면 노랑 ⚠"확인이 필요해요")      │
└─────────────────────────────────────────────────────────────────────────┘
...
**"고아 라벨 관계" 지표 추가 (2026-08-04, PD — `docs/planning.md` "지식그래프 엣지 노출 규칙 최종 확정: 라벨 AND 기간" 4번 PD 인계 항목)**

지식그래프 엣지 노출 규칙이 "라벨 있음 AND 선택 기간 내 공동언급 ≥ 1건"으로 바뀌면서(위 GRAPH-001 "데이터 모델" 절 참고), 라벨은 있지만 **근거 뉴스가 삭제됐거나 태깅이 누락돼 어느 기간에서도 공동언급이 0건인 `OrgRelation`**은 이제 어떤 기간을 선택해도 화면에 나타나지 않는다. RA가 큐레이션한 관계 자산이 조용히 화면에서 사라진 상태라, 검증 게이트의 "조용한 빈 화면" 위험과 같은 이유로 관측 지표를 둔다.
...
- **왜 시간(2일 등) 기준 3단계가 아니라 단순 0/그 외 2단계인가**: 미검증 뉴스와 달리 "고아 라벨"에는 자연스러운 시간축이 없다(발생 이후 며칠이 지났는지가 아니라, "지금 이 순간 몇 건이 안 보이는 상태인가"만 의미가 있다). 검증 상태를 미검증/검증됨 2단계로만 두고 "판정 유예를 제도화하지 않는다"고 확정한 것과 같은 논리로, 여기서도 단순 이진 강조(0건=clear / 1건 이상=attention)만 둔다.
  - `orphan_relation_count == 0` → **clear**: 강조 없음, 초록 `check-circle` 아이콘 + "조용히 잘 보이고 있어요" 캡션(안심시키는 신호를 명시적으로 준다 — 기존 "모두 검증을 마쳤어요" 캡션과 동일한 어법).
...
- **카드 테두리는 "이 지표만"이 아니라 카드 전체에 적용**: `unverified_tier == 'stale' or orphan_relation_count` 조건으로 outer 카드 div의 `border-yellow-200`를 확장했다. 새 지표만 위한 별도 테두리를 안쪽 구분선 블록에 다시 그리면 카드 안에 테두리가 두 겹(바깥 카드 + 안쪽 블록)으로 보여 어색하다 — "같은 카드 안에 얹는다"는 원 설계 취지에 맞게, 미검증이든 고아 라벨이든 "확인이 필요한 것"이 하나라도 있으면 카드 전체가 노란 테두리를 두르는 하나의 신호로 통일했다.
- **레이아웃은 3개 지표와 같은 2줄 스택이 아니라 1줄 압축형**: 라벨(`고아 라벨 관계 (지식그래프)`)을 한 줄에, 값과 캡션(`0건 · 확인이 필요해요/조용히 잘 보이고 있어요`)을 같은 줄에 이어 붙였다. 기존 3개 지표는 각각 라벨 줄 + 값 줄 + 캡션 줄로 최대 3줄을 차지하는데, 4번째 지표까지 같은 방식을 쓰면 카드가 눈에 띄게 길어진다. 지표가 1개뿐이라 "값이 뭔지" 헷갈릴 여지가 적어 압축해도 정보 손실이 없다고 판단했다 — 밀도 문제에 대한 답은 "구분선 자체를 없앤다"가 아니라 "구분선 아래 항목을 더 얇게 만든다"였다.

**현재 값 0(clear) 상태에 대한 판단**: 현재 미검증 지표는 51건이라 stale(노랑 경고)로, 고아 라벨 관계는 0건이라 clear(초록 체크)로 같은 카드 안에서 서로 다른 색 신호가 공존한다. 이건 의도된 정상 동작이라고 본다 — 두 지표는 성격이 다른 독립 신호(하나는 "검증이 밀렸다", 하나는 "라벨 자산이 안 보인다")이고, 각 신호는 자기 값 바로 옆에 아이콘·캡션이 붙어 있어 어느 지표가 경고고 어느 지표가 정상인지 헷갈리지 않는다. 여러 상태 모니터링 카드가 한 화면에서 "이 항목은 정상, 저 항목은 경고"를 동시에 보여주는 건 흔한 패턴이고, 오히려 모든 지표가 항상 같은 색으로 뜨면 "진짜 경고"의 신호 가치가 떨어진다. 다만 값이 실제로 발생했을 때(고아 라벨 1건 이상) 카드 전체가 노란 테두리로 바뀌는 걸 RA가 한 번 보고 확인해 보는 걸 권한다.

**PE 컨텍스트 변수 계약(News 상태 필드·고아 라벨 집계는 PE가 아직 구현하지 않음 — 아래 이름으로 뷰가 채워야 템플릿이 동작한다)**
...
| `orphan_relation_count` | int | 신규(2026-08-04). 전체 기간 기준으로도 두 기업이 함께 언급된 검증 뉴스가 0건인 `OrgRelation`(라벨 있는 관계) 건수. `unverified_count`와 이름 짓는 방식을 그대로 따름(`{상태}_{대상}_count`) — count 쿼리 하나라 비용이 낮다(`docs/planning.md` 4번) |
...
0. (탭 위, 상시 노출) "검증 파이프라인 현황" 카드 — 작은 라벨 "운영자 전용" + 안내문("RA가 아직 판정하지
   ...
   그 아래 얇은 구분선을 두고 "고아 라벨 관계" 지표 1개 추가 — 0건이면 초록 체크 아이콘 +
   "조용히 잘 보이고 있어요", 1건 이상이면 노란 경고 아이콘 + "확인이 필요해요"(지식그래프에서
...
- 검증 파이프라인 현황 카드: 평상시 bg-white border 없음, 미검증 2일 이상 경과 시에만
  ...
  "확인 필요" 톤이므로 노랑을 쓴다. 고아 라벨 관계 지표도 1건 이상이면 동일 팔레트를 쓴다.
```
