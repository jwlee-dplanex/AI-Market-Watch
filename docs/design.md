...
**사이드바 메뉴 라벨과 화면 헤더 제목이 일부 다른 것 — 현행 유지 확정 (2026-08-05)**
...
사용자가 이 불일치를 발견하고 통일 여부를 물었으나, **손대지 않기로 확정**했다. 판단 근거는 사용자 표현 그대로다 — **"통일성 있게 할 것 아니면 그냥 그대로 두는 게 괜찮다."**

- 헤더 쪽을 줄이는 안(`전체 대시보드`→`대시보드`, `AI 인사이트 보고서`→`보고서`)은 이 두 건만 맞추고 끝나지 않는다. `뉴스 상세`·`보고서 상세`의 "상세"는 목록과 구분하는 실질 정보라 성격이 달라서 남게 되고, 그러면 **어디까지가 규칙인지 여전히 애매**하다. 전면 정리가 아니면 고치는 값이 안 나온다.
- 사이드바를 늘리는 안은 폭(240px)과 훑기 효율을 해친다 — 내비게이션 라벨은 읽는 게 아니라 위치를 기억하는 용도라 짧을수록 낫다.

⚠️ **다시 제안하지 말 것.** 이 불일치는 눈에 띄어서 주기적으로 재발견되는 종류이고, 실제로 2026-08-05에 그렇게 제기됐다. 되돌리려면 **5개 화면 + 상세 화면 제목 전체를 하나의 규칙으로 정리하는 안**을 함께 가져와야 한다.
