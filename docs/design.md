...
**캔버스 엣지 표기 (옵션 a, 2026-07-31 확정)** — 위 ASCII 다이어그램에는 지면상 생략했지만, 옵션 a에서는 화면에 뜨는 **모든 엣지가 라벨 엣지**다(라벨 없는 공동언급 전용 엣지는 렌더 대상에서 완전히 제외 — 위 "선 굵기 = 공동등장 가중치, 클릭 가능" 표기도 이제 "굵기 = 선택 기간 활동 강도" 의미로 통일해 읽는다). 실제 캔버스에서는 `금융사●━━[기술협업]━━●AI기업`처럼 **Primary 색(`#60269E`) 실선** 위, 엣지 중점에 **흰 배경 + primary 테두리 pill**로 라벨 텍스트가 클릭 없이 항상 떠 있다. 점선·회색 미분류 실선 등 라벨 유무를 구분하던 이중 채널은 폐기됐다(모든 엣지가 라벨 엣지이므로 구분할 대상 자체가 없음). 상세 스펙은 아래 "라벨 엣지 상시 노출 + 실선 통일 (옵션 a, 2026-07-31)" 절 참고.
...
- 노드 클릭: (1) 클릭한 노드와 인접 노드만 `opacity: 1`, 나머지는 `0.08`로 페이드 — 연결선은 색 자체가 항상 `#60269E`(Primary, 옵션 a로 모든 엣지 통일 색)이므로 강조/톤다운은 색 전환이 아니라 `edgeFadeOpacity`를 통한 투명도 조절로 구현한다(2026-07-28 이전엔 미분류 엣지 기본색 `#D1D5DB`로 톤다운했으나 옵션 a 이후 대상이 없어 폐기). (2) 동시에 `htmx.ajax('GET', '/graph/orgs/<pk>/panel/?period=' + CURRENT_PERIOD, {target:'#org-panel', swap:'innerHTML'})`로 우측 패널을 로드. 배경(빈 공간) 클릭 시 하이라이트 초기화.
...
- **엣지 시각 채널 (옵션 a, 2026-07-31 확정 — 이중 채널 폐기)** — 모든 엣지가 라벨 엣지이므로 라벨 유무를 구분하던 점선/강조색 이중 채널은 폐기하고, **실선 + Primary(`#60269E`) 통일**로 단순화한다. 굵기(`edgeWidth`)만 선택 기간 활동 강도(`value`)를 반영하고, 톤다운 시 페이드 투명도(`edgeFadeOpacity`)는 라벨 엣지 기준 값 하나로 통일한다(`edgeIdleColor`/`edgeIdleOpacity`/`edgeFadeOpacity`/`edgeWidth` 4개 헬퍼는 유지하되 내부의 라벨 유무 분기는 제거). 라벨 텍스트 자체는 흰 배경 pill(`edge-label-g` — `<rect>`+`<text>`)로 엣지 중점에 클릭 없이 상시 렌더한다(대상은 이제 `edges` 전체). 상세 스펙은 아래 "라벨 엣지 상시 노출 + 실선 통일 (옵션 a, 2026-07-31)" 절 참고.
...
1. **엣지 시각 채널 — 실선 통일, 굵기만 기간 반영**:
   ...
   - 색·투명도: 모든 엣지 Primary Violet `#60269E` / opacity `0.75`로 통일(과거 미분류 엣지의 `#D1D5DB` gray-300 톤은 더 이상 쓰지 않는다 — 화면에 미분류 엣지가 없으므로).
   - 점선(`stroke-dasharray`): **폐기**. 모든 엣지 실선. 라벨 유무를 구분할 대상이 없으므로 이중 채널 자체가 무의미해졌다.
   - 강조(하이라이트) 상태: 노드/엣지 클릭 시 비인접 요소 페이드는 `edgeFadeOpacity` 단일 값(과거 "라벨 있음" 분기였던 `0.15`)으로 통일한다(과거 미분류용 `0.05`는 대상이 없어 폐기).
   - 코드: `edgeIdleColor(d)`/`edgeIdleOpacity(d)`/`edgeFadeOpacity(d)`/`edgeWidth(d)` 4개 헬퍼는 그대로 유지하되, 내부의 라벨 유무 분기(`d.has_label ? ... : ...`)를 제거하고 항상 단일 값을 반환하도록 단순화한다.
...
2. **엣지 색상 — 옵션 a(2026-07-31) 이후 `#60269E` 단일 색으로 통일** — 과거엔 미분류 엣지 기본색으로 `#D1D5DB`(Tailwind `gray-300`, 디자인 시스템 1.1 표의 어느 그레이 토큰과도 일치하지 않는 별도 값)를 썼으나, 옵션 a에서 미분류 엣지 자체가 화면에서 사라지므로 이 값은 더 이상 쓰이지 않는다(코드에서 제거 권장). 남은 강조색 `#60269E`(Primary)는 토큰과 일치하며, 이제 이 색 하나가 모든 엣지의 유일한 색이 된다 — AI 카테고리 노드 채움색(위 1번 `CAT_COLORS[2]`)과 동일한 hex를 공유하게 되므로, 엣지 선과 AI 노드가 완전히 같은 색이 된다는 잠재적 혼동 가능성은 참고로 남긴다(선과 원이라는 형태 차이로 구분 가능해 즉각 조치가 필요한 수준은 아니다).
...
- 좌측: 그래프 캔버스(flex-1, 흰 카드, 화면 높이 꽉 채움)
  ...
  - 중앙: force-directed 그래프. 노드=기업(카테고리별 색상 원, 크기는 뉴스 건수 비례 14~40px), 엣지=화면에 뜨는 모든 선이 라벨 있는 관계(라벨 없는 공동언급 전용 선은 렌더하지 않음) — Primary #60269E 실선, 굵기는 선택 기간 공동언급 값에 비례(최소 1.5px 바닥, 값 0이어도 보이는 굵기 유지, 최대 5px), 엣지 중점에 흰 배경/primary 테두리 pill로 라벨 텍스트("기술협업" 등) 클릭 없이 상시 표시, 선택 기간과 무관하게 라벨 엣지는 항상 노출 + 클릭 가능(얇은 선 위에 넓은 투명 히트 영역)
