...
- **표지 배경과 오브를 따로 보지 않는다. 둘을 한 문장으로 묶는다.** AI Market Watch 적용례: **격자 = 시장(데이터가 깔린 판)** / **오브 = 그 위를 지켜보는 렌즈**. 렌즈가 격자를 훑고 **지나간 자리가 잠깐 밝아진다** — 부제 *"지속적으로 수집·요약합니다"*를 시간으로 재현한 것이다. **격자를 그냥 무늬로 두면 오브도 장식으로 남는다.**
- **로고와 은유가 겹치는 것은 좋다.** 상단바 심볼(정지된 돋보기) → 표지 렌즈(움직이는 돋보기) → 이름 `Watch`가 한 줄로 이어진다.
- ⚠️ **다음 장이 할 말을 표지가 먼저 쓰지 않는다.** "점들이 들어와 걸러진다" 안이 있었으나 폐기했다 — 그건 01장 `FIG 1-1`의 몫이라 표지가 먼저 쓰면 01장이 김빠진다. **표지는 "지켜본다"까지만** 말한다.

**주기는 서로 나누어떨어지지 않게 준다** (이 부분은 유지).

```css
.lens-ring.r1{ /* 렌즈 테 — 돌지 않는다. 렌즈는 강체다 */ }
.lens-ring.r2{ animation:lensSpin 46s linear infinite; }
.lens-ring.r3{ animation:lensSpin 24s linear infinite reverse; }   /* 방향도 반대 */
.lens-glass  { animation:lensBreathe 13s ease-in-out infinite; }
```
⚠️ **같은 주기를 주면 링이 한 덩어리로 뭉쳐 돈다.** 서로 어긋나는 값이라야 계속 다른 모양이 나온다. **단 안쪽 렌즈 테는 고정한다** — 링 세 개가 모두 돌면 계측기가 아니라 장식으로 되돌아간다.

**⑶-1 훑기를 구현할 때의 함정 두 가지 (실제로 밟은 것)**

- ⚠️ **밝아지는 격자를 움직이지 말 것.** "지나간 자리가 밝아진다"를 밝은 격자 레이어를 이동시켜 만들면 **아래 격자와 타일이 어긋나 이중선**이 보인다. **격자 레이어는 고정하고 그 위의 `mask-position`만 움직인다.**
- ⚠️ **렌즈와 밝아지는 자리를 각각 다른 단위로 움직이면 어긋난다.** 크기를 CSS 변수(`--lens`·`--scan`)로 두고 **두 키프레임이 같은 식으로 중심 좌표를 계산**하게 하면, 반응형으로 크기를 바꿔도 둘이 절대 어긋나지 않는다.
```css
.cover{ --lens:400px; --scan:660px; }
@keyframes mwLens{ 0%{transform:translateX(calc(10px - var(--lens)/2))} … }   /* 중심 = 10px */
@keyframes mwScan{ 0%{mask-position:calc(10px - var(--scan)/2) 50%} … }       /* 중심 = 10px */
```
- ⚠️ **정지 상태(reduce·print)에서 훑는 도중에 멈추면 이상하다.** 두 미디어 쿼리에서 **렌즈를 판 위 한 지점에 세워** "지켜보는 장면"으로 완성시킨다. **인쇄에서는 훑고 지나간 자리(마스크 레이어)를 빼는 것이 안전하다** — 마스크는 인쇄 엔진마다 결과가 갈린다.
