...
| ④ 주요 이슈 | `<h2>주요 이슈</h2>` 옆 | `dashboard.insights` | "리서치 애널리스트 에이전트가 작성한 주요 이슈 8건을 보여드려요. 각 이슈는 근거 뉴스의 최근 발행일 순으로 정렬해요." |
| ⑤ 최신 뉴스 | `<h2>최신 뉴스</h2>` 옆(우측 "전체 보기" 링크보다 왼쪽) | `dashboard.latest_news` | "선택한 기간에 발행된 뉴스 중 최신순으로 10건을 보여드려요." |
