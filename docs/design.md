...
```
┌─ 헤드라인 (RA 지정 최대 2건 — 2026-08-07 3건→2건, 0건이면 이 구역 전체가 렌더되지 않음) ────┐
...
│                                                                                                │
│  🔥 헤드라인 ⓘ                                                                                │
│  ┌─ 카드 1 (1열 2행 — 2026-08-07 가로 3열→세로 2행, flex-col) ───────────────────────────────┐│
│  │ border-l-4 border-primary                                                                 ││
│  │ 제목 — "주요 이슈" 카드 제목과 동일 클래스, 단 truncate만 제외(자연 줄바꿈)               ││
│  │ 요약 2줄(line-clamp-2, 접힘) · [등급 배지] 관련 기사 N건 · chevron                         ││
│  │ (클릭 시 "주요 이슈"와 동일한 아코디언 펼침 — 분석(display_content 축약본) + 관련 기사,   ││
│  │  시사점 블록은 2026-08-07 제거)                                                            ││
│  └────────────────────────────────────────────────────────────────────────────────────────────┘│
│  ┌─ 카드 2 (카드 1과 완전히 동일 스타일, 카드 1 바로 아래) ──────────────────────────────────┐│
│  └────────────────────────────────────────────────────────────────────────────────────────────┘│
└────────────────────────────────────────────────────────────────────────────────────────────────┘
  ※ (2026-08-07 갱신) 순서(1·2)는 headliner_order 오름차순 위→아래 배치. **등급 배지는
  ...
  ※ (2026-08-07 갱신) 카드가 세로 1열 2행으로 바뀌면서 "좁은 화면 md:flex-row" 반응형 분기는
    제거됐다(모든 화면 폭에서 항상 flex-col) — 아래 "헤드라인 1열 2행 전환" 절 참고.
...
│  │ - 이슈 제목 / [등급 배지] 관련 기사 N건 / 요약 2줄(접힘) │ │  │  │ - 제목 2줄 / 발행일               │ │
...
0. **`INFO_TOOLTIPS["dashboard.headliner"]` 본문의 "최대 3건" → "최대 2건" 정정** — 위 "헤드라인 상한 3건 → 2건" 절 참고. 바로 위 "Info Tooltip 문구" 절 코드블록(825~828행)에 적힌 "이번 주 이슈 중 가장 눈여겨봐야 할 이슈를 **최대 3건** 골라 보여드려요"는 실제 상한이 2건으로 바뀌면서 사실과 어긋난다. `apps/dashboard/tooltips.py`의 `INFO_TOOLTIPS["dashboard.headliner"]` 문자열에서 그 숫자만 "2건"으로 고치면 된다(뒷문장은 그대로).
