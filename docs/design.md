...
> 이 절은 축 1 확정 이전(공동언급 전량 계산 → 라벨 사후 매핑) 방식을 아래 옵션 a 방식으로 대체한다. PE 미구현 상태이므로 실제 `apps/graph/views.py` 코드는 여전히 구방식일 수 있다 — 아래가 목표 스펙.
