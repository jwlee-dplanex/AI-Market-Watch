...
### 1.6 로고 · 심볼 · 파비콘 (2026-08-04 PD 초안, 2026-08-04 파비콘 우선 기준으로 개정, 상태: **미확정 — 후보 3안**)
...
후보 A · 슬래시 액센트 — 바이올렛 풀블리드 배경 + 초록 대각선 슬래시 (도형 2개)
후보 B · 지식그래프 노드-엣지 — 바이올렛 풀블리드 배경 + 초록/흰 노드 3개·엣지 2개 (단순 위상)
후보 C · 관찰(Watch) 은유 — 바이올렛 풀블리드 배경 + 흰 돋보기 실루엣 + 초록 렌즈 (도형 2개)
  → 프롬프트 원문·채택 근거는 PD 응답(2026-08-04) 참고. 채택안이 확정되면 이 절에
    최종 프롬프트·이미지 경로·확정 수치(최소 크기, 여백, 금지 변형)를 옮겨 적는다.

PD 추천(미확정 상태의 잠정 의견): 후보 A(슬래시 액센트). 1.1 컬러 토큰에 이미 "슬래시
      액센트"로 정의된 기존 브랜드 자산을 재사용해 새 디자인 언어를 만들 필요가 없고,
      도형이 2개뿐이라 이미지 생성 성공률(hex 정확도·불필요한 디테일 삽입 위험)이 가장
      낮다. 후보 B는 프로덕트 고유성(지식그래프)이 강하지만 점+선 조합은 생성 모델이
      두께를 불균일하게 그리는 실패가 잦아 검수·재시도 비용이 더 든다. 최종 채택은
      사용자 확인 후 확정한다.

사용 규칙 (채택안 공통 적용 — 심볼 확정 후 이미지 실측값으로 교체):
  ...
  배경 — 심볼 자체가 바이올렛 풀블리드 배경을 포함하므로 밝은/어두운 탭 모두 별도 배경
        지정 없이 사용
  금지 — 색상 임의 변경, 도형 각도·개수 변형, 그림자·아웃라인·수동 모서리 라운딩 추가,
...
**PE 컨텍스트 변수 계약(News 상태 필드는 PE가 아직 구현하지 않음 — 아래 이름으로 뷰가 채워야 템플릿이 동작한다)**
...
**(2026-07-31 옵션 a 추가 참고)** 라벨 엣지는 기간과 무관하게 상시 노출되므로, 라벨이 하나라도 있으면 어떤 기간을 선택해도 "연결 0개" 빈 상태에 도달하지 않는다(엣지 0개 빈 상태는 라벨이 전혀 없을 때만 발생). 반대로 "기업 0개"는 여전히 발생 가능하다 — 라벨 엣지가 있어도 그 엣지가 참조하는 조직이 `is_active=False`로 바뀌면 노드 union에서도 제외되기 때문이다(아래 "데이터 모델" 절의 union 정의는 `is_active=True`를 전제로 한다).
...
**데이터 모델 (`apps/graph/views.py`) — 옵션 a 갱신 (2026-07-31, 아래가 현행 최종 스펙)**

> 이 절은 축 1 확정 이전(공동언급 전량 계산 → 라벨 사후 매핑) 방식을 아래 옵션 a 방식으로 대체한다. **PE 구현 완료(2026-07-31, `apps/graph/views.py`)** — 아래는 목표 스펙이자 현행 코드의 설명이다.

- **엣지 존재 게이트 = `OrgRelation` 존재(라벨 있음)** — 엣지 후보는 `OrgRelation.objects.all()`(정규화된 `(org_a, org_b)` 쌍) 중 `ALLOWED_TYPE_PAIRS = {frozenset({"금융사","AI"}), frozenset({"보험사","AI"})}`를 만족하는 것만 채택한다(기존 타입 제약은 그대로 유지 — 업계 관계망이라는 화면 목적상 "동종업계끼리의 단순 동반 언급"보다 금융/보험-AI 교차 관계에 집중하기 위한 설계 판단). 공동언급만 있고 `OrgRelation`이 없는 쌍은 이제 엣지 딕셔너리 자체를 만들지 않는다(`itertools.combinations`로 모든 공동언급 쌍을 전수 계산하던 기존 방식은 폐기).
- **엣지 굵기(`value`)** — 위에서 확정된 라벨 엣지마다, 선택 기간 내 두 기업이 같은 `News`에 함께 태깅된 건수를 계산한다(기존 combinations 집계 로직을 "라벨 엣지 쌍에 한정"해 재사용). 기간 내 공동언급이 없으면 `value=0`이며, 이 경우에도 엣지는 사라지지 않는다(상시 노출 정책, 아래 시각화 절의 최소 굵기 바닥 참고).
- **노드 union** — 노드 목록은 기존 조건(`Organization.objects.filter(is_active=True).annotate(news_count=Count("news", filter=...)).filter(...)`, `symbolSize = max(14, min(40, 14 + news_count*2))`)에 **라벨 엣지 양끝 조직을 OR로 합친다**: `Q(news_count__gt=0) | Q(pk__in=labeled_edge_org_pks)`. 즉 선택 기간에 뉴스가 0건인 조직도 라벨 엣지의 상대편이면 노드로 노출한다(안 그러면 존재하지 않는 노드를 참조하는 떠다니는 엣지가 생김). `is_active=False`인 조직은 이 union에도 포함되지 않는다(비활성 기업은 예외 없이 노드에서 제외).
...
**라벨 엣지 상시 노출 + 실선 통일 (옵션 a, 2026-07-31 확정 — 이전 "기간 내 실존 엣지의 라벨 표시" 절 전체 대체)**

> 이 절은 축 1 "기간 필터 예외 → 롤백 → 옵션 a 재확정"의 최종 결과다(위 GRAPH-001 서두 개정 이력의 2026-07-31 문단 참고). 이전 절(2026-07-28 최초 구현 → 2026-07-28 정정)이 서술한 "실존 엣지에 라벨 사후 매핑 + 점선/실선 이중 채널"은 **전부 폐기**한다. `templates/graph/index.html`·`apps/graph/views.py`는 이 문서 갱신 시점에 아직 옵션 a를 반영하지 않은 상태(PE 구현 대기) — 아래가 PE가 구현할 목표 스펙이다.

옵션 a는 두 개의 결합된 규칙이다: (1) **엣지 존재 자체가 라벨(`OrgRelation`) 유무로 결정**된다 — 위 "데이터 모델" 절대로, 공동언급만 있고 라벨이 없는 쌍은 애초에 엣지 딕셔너리가 생성되지 않는다. (2) 그렇게 존재하는 라벨 엣지는 **선택 기간과 무관하게 상시 노출**되며, 굵기만 선택 기간의 공동언급 `value`를 반영한다(0 가능).

1. **엣지 시각 채널 — 실선 통일, 굵기만 기간 반영**:
   - 굵기: `Math.max(1.5, Math.min(1 + d.value * 0.5, 5))`. `value=0`이어도 1.5px 바닥이 적용돼 캔버스에서 사실상 안 보일 만큼 얇아지지 않는다(옵션 a에서는 이 바닥이 실제로 자주 발동한다 — 기간 무관 상시 노출 정책상 `value=0`인 라벨 엣지가 정상적으로 존재하기 때문이다. 2026-07-28 롤백 직후엔 이 시나리오가 없어 죽은 코드였지만 옵션 a로 다시 살아난다).
...
3. **`apps/graph/views.py`의 `graph()` 뷰가 채우는 엣지 JSON 필드 (옵션 a 목표 스펙, 2026-07-31)**:
   ...
   - `source`/`target`은 변경 없음. `value`는 "선택 기간 공동언급 건수"만 의미하며, **0이 정상적으로 발생**한다(2026-07-28 롤백 직후엔 `value=0`인 라벨 엣지가 구조적으로 없었으나, 옵션 a는 라벨 엣지를 기간 무관 상시 노출하므로 자연 발생한다). 라벨이 있다고 값을 부풀리지 않는다는 원래 계약은 유지된다.
...
4. **노드 union 방어 로직 재확인** — `symbolSize = max(14, min(40, 14 + news_count*2))`의 14px 하한은 `news_count=0`인 union 노드(라벨 엣지 상대편, 위 "데이터 모델" 절 참고)에도 그대로 적용돼 최소 크기로 안전하게 그려진다. 2026-07-28 롤백 직후엔 union 자체가 없어 이 시나리오가 발생하지 않았지만, 옵션 a에서는 실제로 발생하므로 이 코드가 다시 살아있는 방어 로직이 된다는 점을 확인해둔다.
