...
**⚠️ 화면 최상단이 "핵심 지표"가 아니라 "주요 이슈 + 최신 뉴스"다 (2026-08-06 확정, PD).** 종전에는 "핵심 지표"(일별 추이/기업별 Top10/기술주제별)가 최상단, "주요 이슈 + 최신 뉴스"가 그 아래였다. 사용자 확정으로 순서를 뒤집었다 — 곧 상단에 들어올 헤드라이너(주요 이슈 중 가장 중요한 최대 3건, `docs/planning.md` "대시보드 헤드라이너" 절, 이번 스코프 아님)를 포함해 **사용자가 대시보드를 열었을 때 먼저 보고 싶은 건 지표가 아니라 "이번 주 무슨 일이 있었나"**라는 판단이다. 지표는 그걸 본 뒤 확인하는 보조 자료로 재배치했다. `templates/dashboard/index.html`은 이미 이 순서로 구현 완료(마크업 자체는 변경 없이 두 최상위 블록의 파일 내 위치만 맞바꿨다).
...
│  │ - 이슈 제목 / 관련 기사 N건 / 요약 2줄(접힘)             │ │  │  │ - 제목 2줄 / 발행일               │ │
│  │ - 펼치면 content 전문 + 시사점 + 관련 기사 링크(→NEWS-002)│ │  │  │                                   │ │
