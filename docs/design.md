...
**AI Market Watch 카테고리 색상 매핑**

| 카테고리 | 색상 | 용도 |
|----------|------|------|
| 기술 흐름 | Violet `#60269E` | AI 기술 트렌드 |
| 기업 사례 | Blue Green `#00AF9A` | 기업 도입 사례 |
| 금융권 활용 | `#1D4ED8` (blue-700) | 금융 특화 콘텐츠 |
| 규제·정책 | Orange `#FF6C0E` | 규제·법률 동향 |
| 경쟁사 동향 | Dark Gray `#54565A` | 경쟁사 분석 |
...
- 사이즈: 12px (인라인 pill), 16px (버튼·네비), 18px (카드 내부), 28px (카테고리 카드), 32px (헤더·보드)
...
- 배지: 카테고리별 soft 색상 (기술흐름 #F3EAFB/#60269E, 금융권 #EEF2FF/#1D4ED8, 규제 #FFF3EC/#FF6C0E)
- 한국어 샘플 데이터 사용
```
...
1. 상단 검색 + 필터 바
   ...
   - 드롭다운 필터: 기간(오늘/이번주/이번달), 출처(네이버/OpenDART/RSS), 카테고리, DPLANEX태그
   - 필터 초기화 버튼
...
3. 뉴스 카드 5개 (리스트 형태, 각 카드 세로 배치)
   카드 구성:
   - 상단: 카테고리 배지 + DPLANEX 태그 배지 + 출처·발행일 (우측 정렬)
   - 제목 (Source Serif 4, 18px)
   - 요약 2줄 (Inter 14px, text-gray-600)
   - 하단: "유사 기사 5건" 링크 + "상세보기" 버튼

   샘플 데이터:
   - "삼성SDS, AI Agent 플랫폼 도입으로 고객 상담 자동화 85% 달성" / 기업 사례 / AI Agent / ZDNet Korea / 2시간 전
   - "금융위원회, 생성형 AI 활용 가이드라인 초안 발표" / 규제·정책 / RAG / 금융위원회 RSS / 3시간 전
   - "Anthropic, Claude 4 출시…코딩·분석 능력 대폭 강화" / 기술 흐름 / AI Platform / 연합뉴스 / 5시간 전
   - "KB국민은행, LLM 기반 여신심사 시스템 도입 추진" / 금융권 활용 / Data Platform / 매일경제 / 어제
   - "국내 AI 스타트업 투자 유치 현황 Q2 결산" / 경쟁사 동향 / 없음 / 한국경제 / 어제

4. 페이지네이션 (1, 2, 3 ... 25)

[스타일 규칙]
- 카드 hover: shadow 강해짐, border-color #60269E 으로 변경
- 카테고리 배지: 각 카테고리 soft 색상
- DPLANEX 태그 배지: bg #F3EAFB, text #60269E, 작은 크기
```
...
2. 메인 콘텐츠
   - 카테고리 배지(기업 사례) + 출처(ZDNet Korea) + 발행일
   ...
   - DPLANEX 인사이트 섹션 (H3)
     callout 2개:
     · "KANDLE 관점: 금융권 콜센터 AI 자동화 수요 증가. KANDLE 금융의 Agent 모듈 고도화 기회."
     · "AI Studio 관점: 고객사 AI Agent 구축 레퍼런스로 활용 가능한 아키텍처 사례."

3. 사이드 패널 (sticky)
   - DPLANEX 태그 섹션: 배지 형태로 "AI Agent", "RAG", "기업 사례"
...
┌─ DPLANEX 시사점 ────────────────────────────────────────┐
│  KANDLE / AI Studio / 관계사 AX 관점별 callout         │
└─────────────────────────────────────────────────────────┘
...
4. DPLANEX 시사점 섹션 (H2)
   ...
   - KANDLE 관점 (border #60269E)
   - AI Studio 관점 (border #00AF9A)
   - 관계사 AX 지원 관점 (border #93D500)
...
**목적**: 뉴스 수집 대상 API·RSS·크롤링 소스를 추가·수정·활성화 관리
...
┌─ 소스 테이블 ───────────────────────────────────────────┐
│  소스명  │  URL  │  유형  │  수집 주기  │  활성  │ 액션 │
├──────────┼───────┼────────┼────────────┼────────┼──────┤
│  네이버   │  ...  │ [API]  │  매일 09:00│  ON ●  │ ✎ ⊘ │
│  금융위   │  ...  │ [RSS]  │  매일 09:00│  ON ●  │ ✎ ⊘ │
│  OpenDART│  ...  │ [API]  │  매일 09:00│  ON ●  │ ✎ ⊘ │
│  Anthropic│ ...  │ [크롤] │  매일 10:00│  ON ●  │ ✎ ⊘ │
│  Google  │  ...  │ [크롤] │  매일 10:00│  OFF ○ │ ✎ ⊘ │
│  한국은행 │  ...  │ [RSS]  │  매일 09:00│  ON ●  │ ✎ ⊘ │
└──────────┴───────┴────────┴────────────┴────────┴──────┘
```
...
**목적**: 수집 대상 키워드와 제외 키워드를 추가·삭제 관리
